...
/**
 * 展示詳細ページ
 * - 画像パスの修正: publicフォルダ参照用に先頭に / を追加
 * - レイアウト修正: 長文でもノートが切れないように h-fit と余白を追加
 */
export default function ShoeDetailPage() {
  ...
  // ■■■■■ データ編集エリア ■■■■■
  // 画像ファイルは public フォルダに入れて、パスの先頭に / を付けてください
  const shoesData: Record<string, any> = {
    ...
    "2": {
      ...
      author: "BLOOM LAB",
      note: "#2026_AUGUST",
      description: "「Bloom Lab(ブルームラボ)」はクリエイター体験デジワークを通じて子どもたちの未来を広げるプロジェクト\n\n様々な子ども食堂さんなどでイベントを開催してきました。今回の展示ではそんな体験を少しだけ体験していただければと思います。",
      image: "/blm.png", // 先頭に / を追加
      color: "bg-[#ffebee]",
          ...
          <div className="w-full min-h-full flex flex-col items-center justify-start pt-24 pb-20 px-6 md:justify-center md:pt-0 md:pb-0">
            ...
            >
              <div className="bg-[#ffebcd] px-8 py-2 md:px-12 md:py-3 shadow-md transform -skew-x-12 border-l-4 border-r-4 border-[#ffebcd]/50 relative">
                ...
                <h1 className="text-[#5a6c7c] text-2xl md:text-4xl font-black tracking-widest italic transform skew-x-12 uppercase text-center leading-tight">
                  {currentData.title}
                </h1>
              </div>
            </motion.div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-8 w-full max-w-5xl">
              ...
              <motion.div
                ...
                // h-fit を追加して内容に合わせて伸びるようにし、pb-10 で下の余白を確保
                className="w-full md:w-[35%] bg-white border-l-4 border-gray-200 p-6 shadow-lg rotate-1 relative min-h-[200px] h-fit pb-10 flex flex-col"
              >
                {/* ノートの穴 */}
                ...
                {/* フッター */}
                <div className="mt-8 pt-2 border-t border-gray-100 flex justify-between items-center">
